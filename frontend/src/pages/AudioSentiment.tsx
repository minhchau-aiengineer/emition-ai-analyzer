...
const WaveformCanvas: React.FC<{ audioBuffer?: AudioBuffer; height?: number }> = ({
    ...
    useEffect(() => {
        ...
        if (!audioBuffer) {
            // draw subtle placeholder
            ctx.globalAlpha = 0.2;
            for (let i = 0; i < width; i += 8 * dpr) {
                const barH = (Math.sin(i * 0.01) * 0.5 + 0.5) * (h * 0.35);
                const y = h / 2 - barH / 2;
                ctx.fillRect(i, y, 5 * dpr, barH);
            }
            ctx.globalAlpha = 1;
            return;
        }
        ...
        <div className="h-3 w-full rounded-full bg-slate-700/60 border border-white/10 overflow-hidden">
            <div
                ...
                style={{
                    ...
                    background:
                        "linear-gradient(90deg, rgba(14,165,233,1) 0%, rgba(99,102,241,1) 100%)",
                }}
            />
        </div>
    );
...
const ResultCard: React.FC<{
    ...
    const color = label === "Positive" ? "text-emerald-300" : label === "Negative" ? "text-rose-300" : "text-slate-300";
    return (
...
export default function AudioSentiment() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                // Simple quality: RMS measurement
    ...
    // Analyze (demo: mock; replace with real API call)
    const analyze = async () => {
            ...
            <div className={classNames(tokens.card, "p-5 md:p-6")}> 
                ...
                {tab === "upload" && (
                    <div className="space-y-4">
                        ...
                        {file && (
                            <div className="text-sm text-slate-300">Selected: {file.name} · {formatBytes(file.size)}</div>
                        )}
                    </div>
                )}
